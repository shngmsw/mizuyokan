...
impl TextServiceFactory {
            ...
            CompositionState::Composing => match action {
                ...
                UserAction::Enter => {
                    if composition.suffix.is_empty() {
                        (
                            CompositionState::None,
                            vec![ClientAction::Settle, ClientAction::EndComposition],
                        )
                    } else {
                        (
                            CompositionState::Composing,
    ...
    pub fn handle_action(
        ...
        for action in actions {
            match action {
                ...
                ClientAction::AppendText(text) => {
                    ...
                    let synced = if mode == InputMode::Kana {
                        super::mizuyokan::sync(&raw_input, mixed.is_some(), &mut ipc_service, false)?
                    } else {
                        None
                    };
                    candidates = match synced {
                        Some((synced, segments)) => {
                            mixed = segments;
                            synced
                        }
                        None => ipc_service.append_text(text.clone())?,
                    };
                ...
                ClientAction::RemoveText => {
                    ...
                    if mode == InputMode::Kana {
                        super::mizuyokan::prefetch(&raw_input);
                        if let Some((synced, segments)) = super::mizuyokan::sync(
                            &raw_input,
                            mixed.is_some(),
                            &mut ipc_service,
                            false,
                    ...
                    let pick = |list: &Vec<String>| {
                        list.get(selection_index as usize).cloned().unwrap_or_default()
                    };
                    let (text, sub_text) = (pick(&candidates.texts), pick(&candidates.sub_texts));
                    let hiragana = candidates.hiragana.clone();
                    corresponding_count = candidates
                        .corresponding_count
                        .get(selection_index as usize)
                ...
                ClientAction::SetSelection(selection) => {
                    ...
                    if first_conversion {
                        if let Some((settled, segments)) = super::mizuyokan::sync(
                            ...
                            let shown = format!("{preview}{suffix}");
                            let changed = settled
                                .texts
                                .first()
                                .zip(settled.sub_texts.first())
                                .is_some_and(|(t, s)| format!("{t}{s}") != shown);
                            candidates = settled;
                            mixed = segments;
                            ipc_service.set_candidates(candidates.texts.clone())?;
                            if changed {
                                // The live preview was stale; Down below lands on the settled reading.
                                selection_index = -1;
                            }
                        }
                    }
                ...
                ClientAction::ShrinkText(text) => {
                    ...
                    // What is left in azooKey may still hold full-width English;
                    // make the next keystroke rebuild it from the raw buffer.
                    if mixed.is_some() {
                        mixed = Some(Vec::new());
                    }
                    let text = match mode {
        ...
        if let Some(mode) = new_mode {
            if let Err(e) = self.publish_open_state(&mode) {
                tracing::warn!("Failed to publish open/close state: {e:?}");
            }
        }
