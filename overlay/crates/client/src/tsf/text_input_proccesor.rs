...
use super::{
    ...
    key_event_sink::{GUID_PRESERVEDKEY_TOGGLE, PRESERVEDKEY_TOGGLE},
};
use windows::{
    core::Interface as _,
    Win32::{
...
impl ITfTextInputProcessor_Impl for TextServiceFactory_Impl {
    ...
    fn Activate(&self, ptim: Option<&ITfThreadMgr>, tid: u32) -> Result<()> {
        ...
        tracing::debug!("PreserveKey");
        let description: Vec<u16> = "Toggle input mode".encode_utf16().collect();
        unsafe {
            thread_mgr.cast::<ITfKeystrokeMgr>()?.PreserveKey(
                tid,
                &GUID_PRESERVEDKEY_TOGGLE,
                &PRESERVEDKEY_TOGGLE,
                &description,
            )?;
        };
        ...
        tracing::debug!("Advise open/close compartment sink");
        if let Err(e) = self.advise_open_close_sink() {
            tracing::error!("Failed to advise open/close sink: {e:?}");
        }
    ...
    fn Deactivate(&self) -> Result<()> {
        ...
        if let Err(e) = self.unadvise_open_close_sink() {
            tracing::error!("Failed to unadvise open/close sink: {e:?}");
        }

        {
            ...
            unsafe {
                thread_mgr
                    .cast::<ITfKeystrokeMgr>()?
                    .UnpreserveKey(&GUID_PRESERVEDKEY_TOGGLE, &PRESERVEDKEY_TOGGLE)?;
            };
